...
- **Don't reach for the built-in `hyperframes tts` command for Hebrew narration.** The bundled Kokoro-82M supports 8 languages via voice-ID prefix, `a`=American English, `b`=British English, `e`=Spanish, `f`=French, `h`=Hindi, `i`=Italian, `j`=Japanese, `p`=Brazilian Portuguese, `z`=Mandarin. Hebrew is not included. Generate the WAV/MP3 with an external service (ElevenLabs Hebrew voices, OpenAI TTS, Google Cloud TTS Hebrew) and drop the file into the composition as a normal `<audio>` clip.
- **Don't use `.en` Whisper models on Hebrew audio.** `.en` variants TRANSLATE non-English audio to English instead of transcribing it. For Hebrew captions use `npx hyperframes transcribe audio.wav --model small --language he` (or `medium --language he` / `large-v3 --language he` for noisy audio). The `.en` suffix is only correct when the user explicitly says the audio is English.
