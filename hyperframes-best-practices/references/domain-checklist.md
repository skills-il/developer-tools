...
| M11 | Host toolchain: **Node >= 22**, FFmpeg on PATH, and a **Puppeteer-managed `chrome-headless-shell`** that the CLI auto-downloads into its cache. Everything the skill instructs is a shell command. | `packages/cli/package.json:76`, `packages/producer/package.json:119`, `packages/cli/src/browser/manager.ts:191` |
| M12 | `check` is the gate (`lint` + runtime + layout + motion + WCAG contrast in one browser session, `--samples` default **9**); `validate`/`inspect`/`layout` are deprecated aliases; `lint` is not deprecated. | `packages/cli/src/commands/check.ts:41-126`; `packages/cli/src/cli.ts:120-162` |
