...
- **Supavisor** (ה-pooler המשותף של Supabase): הכתובת היא `aws-[INDEX]-[REGION].pooler.supabase.com`, פורט **6543** למצב transaction ופורט **5432** למצב session. `[INDEX]` הוא מספר אשכול ה-pooler ואי אפשר לגזור אותו מהאזור, ולכן העתיקו את המחרוזת המדויקת מחלון ה-Connect של הפרויקט במקום לקבע קידומת כמו `aws-0-`.
- **PgBouncer**: זמין כ-pooler הייעודי של Supabase, וגם הבחירה הרגילה באחסון עצמי.
- גודל ה-pool אינו קבוע לפי תוכנית. זו הגדרה שאפשר לשנות (Database settings בדשבורד), ש-Supavisor ו-PgBouncer קוראים שניהם אבל מיישמים כל אחד בנפרד, והיא מגבילה את מספר החיבורים שה-pooler פותח מול Postgres. מה שכן משתנה לפי דרגת המחשוב הוא תקרת "max pooler clients" נפרדת, שקובעת כמה לקוחות יכולים להתחבר ל-pooler בו זמנית, ובנוסף `max_connections` של המופע. קראו את המספרים מהפרויקט שלכם במקום לקבע מספר כלשהו.
