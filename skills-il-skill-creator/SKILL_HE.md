...
- frontmatter של SKILL.md משתמש ב-YAML עם מבנה מקונן ספציפי (מערכי metadata.tags.he/en). סוכנים עלולים לשטח את התגיות למערך בודד במקום להשתמש במבנה הדו-לשוני he/en.
- תוכן בעברית ב-SKILL_HE.md לעולם לא צריך להופיע בתוך בלוקי קוד (```) כי בלוקי קוד לא תומכים ב-RTL. יש להשתמש בטקסט רגיל או רשימות תבליטים עבור תוכן עברי.
- שדה תיאור הסקיל משמש גם כתיאור ב-frontmatter וגם כטקסט הפעלה להתאמת סוכנים. סוכנים עלולים לכתוב תיאור גנרי שלא מופעל על שאילתות רלוונטיות של משתמשים.
- סקילים חייבים לעבור ולידציה מול הסכמה של skills-il (name, description, license, metadata עם version/category/tags). סוכנים עלולים להשמיט שדות נדרשים כמו supported_agents או display_name.
