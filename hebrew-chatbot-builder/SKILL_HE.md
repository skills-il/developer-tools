...
- מודלים שמטפלים בעברית היטב (נכון לאמצע 2026): Anthropic Claude (דרגות Sonnet/Opus), משפחת OpenAI GPT-5.x, ו-Google Gemini 3.x. שמות המודלים והדרגות משתנים תכופות (GPT-4o/4.1 ו-Gemini 2.x הוצאו משימוש בתחילת 2026), אז בדקו את רשימת המודלים העדכנית של כל ספק לפני שמקבעים אחד.
- לאפשרות עברית-נייטיב, בחנו את Dicta-LM 3.0 (דיקטה, אוניברסיטת בר-אילן) לסיווג on-prem.
- **התגוננו מפני prompt injection (OWASP LLM01).** כשטקסט חופשי של משתמש מגיע ל-LLM שיכול להפעיל פעולות (חיפוש הזמנה, העברה לנציג, פתיחת פנייה), התייחסו אליו כלא-מהימן: עטפו את טקסט המשתמש במפרידים ברורים, הנחו את המודל להתעלם מכל הוראה שבתוכו, לעולם אל תחשפו את הפרומפט המערכתי, ואמתו את הפעולה שהמודל בחר מול רשימת היתר לפני ביצוע.
- **בצעו streaming בווידג'ט הווב, ובאפר ב-WhatsApp/Telegram.** צ'אט ווב צריך לזרום טוקנים להשהיה נתפסת נמוכה; WhatsApp ו-Telegram לא יכולים לזרום הודעה חלקית, אז הציגו אינדיקטור הקלדה ושלחו את התשובה המלאה כשהיא מוכנה.
...
def extract_israeli_entities(text: str) -> dict:
    ...
    # ה-lookarounds על גבולות הספרות קריטיים. בלעדיהם התבנית הקווית תופסת גם את תשע
    # התווים הראשונים של מספר נייד שנכתב ברצף, ולכן "0521234567" מחזיר גם את המספר
    # האמיתי וגם את "052123456" השגוי. כל תהליך שמחייג או שולח הודעה למספר שחולץ יקבל
    # אז סיכוי של 50% להשתמש במספר קטוע. בנוסף התוצאות עוברות ניקוי כפילויות תוך שמירת סדר.
    phone_patterns = [
        r'(?<!\d)05\d[\s-]?\d{3}[\s-]?\d{4}(?!\d)',      # נייד: 050-1234567
        r'(?<!\d)0[2-9][\s-]?\d{3}[\s-]?\d{4}(?!\d)',     # קווי: 02-1234567
        r'(?<!\d)\+?972[\s-]?\d{1,2}[\s-]?\d{3}[\s-]?\d{4}(?!\d)',  # בינלאומי
    ]
    phones: list[str] = []
    for pattern in phone_patterns:
        for match in re.findall(pattern, text):
            # מספר קווי ישראלי שנכתב בלי מפרידים הוא בן תשע ספרות, וכך גם תעודת זהות.
            # ערך שעובר את ספרת הביקורת הוא הרבה יותר סביר שהוא ת"ז ולא טלפון, וסיווג
            # שלו כטלפון גורם לתהליך כלשהו לחייג לתעודת זהות ולשמור אותה בשדה לא רגיש.
...
def is_valid_teudat_zehut(value: str) -> bool:
    ...
    if len(digits) != 9 or not digits.isdigit():
        return False
    # רצף של אפסים עובר את בדיקת המודולו אבל הוא ערך placeholder ולא תעודת זהות. זה הערך
    # הנפוץ ביותר בנתוני בדיקה ובנתוני זריעה, ולכן דוחים אותו במפורש.
    if set(digits) == {"0"}:
        return False
    total = 0
...
לקובץ מוכן להעתקה של ביטויי שיחה בעברית (פתיחה, אישור, עיבוד, הצלחה, שגיאה, fallback, סיום, המתנה, התנצלות) עם תעתיק ומתי להשתמש בכל אחד, ראו `references/hebrew-chatbot-phrases.md`.
...
- **אימות חתימת webhook חייב להיכשל סגור, והבדיקה לא יכולה להיות מותנית בכך שהסוד קיים.** העטיפה שנראית טבעית `if APP_SECRET:` היא הבאג עצמו: בדיקת משתני הסביבה שהייתה תופסת סוד חסר יושבת בדרך כלל תחת `if __name__ == "__main__":`, שלא רץ כלל תחת שרת WSGI (`gunicorn handler:app`), וכך בפועל פורסים webhook כי Meta דורשת HTTPS. התוצאה היא נקודת קצה בייצור שמקבלת כל payload מזויף בזמן שכל בדיקה מקומית עוברת. סרבו לשרת (HTTP 500) כשהסוד חסר.
- **לעולם אל תשלחו לייצור `app.run(debug=True)` על שרת webhook.** הדיבאגר של Werkzeug הוא הרצת קוד מרחוק בתכנון, ושרת webhook הוא מעצם הגדרתו נגיש לציבור. קשרו את שרת הפיתוח ל-`127.0.0.1` והציבו לפניו שרת WSGI אמיתי ו-TLS בייצור.
- **רצף של תשע ספרות אינו תעודת זהות, ותבנית טלפון בלי עוגנים ממציאה מספרים.** אמתו התאמות של ת"ז מול אלגוריתם ספרת הביקורת ודחו את ערך האפסים; עגנו תבניות טלפון ב-lookarounds על גבולות ספרות, אחרת התבנית הקווית תתפוס גם את תשעת התווים הראשונים של מספר נייד רציף ותחזיר מספר קטוע לצד המספר האמיתי.
- **נעצו את גרסת ה-Graph API בקבוע אחד בעל שם.** Meta מוציאה גרסה משימוש כשנתיים אחרי השחרור (גרסה v25.0 מפברואר 2026 נתמכת עד 29 ביולי 2028). פיזור הגרסה בתוך כתובות URL בארבעה קבצים הופך עדכון של שורה אחת לחפירה ארכיאולוגית, וגרסה שנשכחת נכשלת רק בחלק מסוגי ההודעות.
...
| Meta Graph API Changelog | https://developers.facebook.com/docs/graph-api/changelog/ | חלונות תמיכה בגרסאות. כל גרסה יוצאת משימוש כשנתיים אחרי השחרור; גרסה v25.0 (פברואר 2026) נתמכת עד 29 ביולי 2028 |
| Telegram Bot API Docs | https://core.telegram.org/bots/api | מתודות Bot API, מקלדות inline, הגדרת webhook. יומן השינויים בתחתית העמוד מציין את הגרסה הנוכחית (Bot API 10.3, 24 באוגוסט 2026); הסקיל הזה לא נועל גרסה, אז בדקו שם יכולות שנוספו מאז |
