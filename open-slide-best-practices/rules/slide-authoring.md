...
## Hebrew and RTL adaptation

Every concrete style example in the rules above uses physical CSS (`paddingLeft: 48`, `padding: '0 160px'`, `letterSpacing: '0.2em'`, `text-align: 'left'`). For Hebrew or bilingual content:

- Replace `paddingLeft` / `paddingRight` with `paddingInlineStart` / `paddingInlineEnd` (or the shorthand `paddingInline` for symmetric padding).
- Replace `marginLeft` / `marginRight` with `marginInlineStart` / `marginInlineEnd`.
